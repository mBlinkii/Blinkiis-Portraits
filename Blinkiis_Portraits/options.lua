...
local function CreateDropDownMenu(parent, point, value, items, onChange)
	...
	UIDropDownMenu_Initialize(dropDown, function(self, level, menuList)
		...
		for key, name in ipairs(items) do
			info.text, info.arg1, info.checked = name, key, value == key
...
function BLINKIISPORTRAITS:InitializeOptions()
	...
	local function CreatePortraitOptions(name, point, dbField)
		...
		local dropDownMenu = CreateDropDownMenu(self.Options, { "LEFT", extraCheckButton.Text, "RIGHT", 10, 0 }, dbField.texture, portraits, function(value)
	...
	local function CreateExtraOptions(name, point, key)
		...
		local dropDownMenu = CreateDropDownMenu(self.Options, { "LEFT", lable, "RIGHT", 10, 0 }, self.db.misc[key], extra, function(value)
